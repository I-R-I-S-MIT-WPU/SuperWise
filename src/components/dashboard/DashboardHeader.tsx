...
export function DashboardHeader({ user, goalProgress }: DashboardHeaderProps) {
  ...
  const getRiskColor = (risk: string) => {
    switch (risk) {
      ...
      case "High":
        return "bg-danger/15 text-danger-foreground border-danger/30";
      default:
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API}/trigger-email/${newsSource}`, {
        method: "GET",
        headers: {
        ...
        <div className='flex items-center gap-6 flex-1'>
          ...
          <div className='flex-1'>
            ...
            <div className='flex flex-wrap items-center gap-2'>
              <Badge
                variant='outline'
                className='text-sm py-1.5 px-3 font-medium bg-muted/50'>
                Age {user.Age ?? "N/A"}
              </Badge>
              <Badge
                variant='outline'
