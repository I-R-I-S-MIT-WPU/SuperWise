...
export function SummaryCard({
  ...
  const retirementAmount =
    projection?.adjusted_projection || user?.Projected_Pension_Amount || 0;
  ...
  const investmentPercentage = primaryInvestment
    ? investmentTypes[primaryInvestment]?.percentage || 0
    : 0;
    ...
    <div className='summary-card mb-8'>
      ...
      <div className='grid lg:grid-cols-2 gap-6 mb-6'>
        <div className='bg-white/10 backdrop-blur-sm rounded-xl p-6'>
          ...
          <div className='text-2xl lg:text-3xl font-bold text-white mb-3'>
      ...
      <div className='bg-white/10 backdrop-blur-sm rounded-xl p-6'>
        <div className='flex items-center gap-3 mb-4'>
          ...
          <h3 className='text-1xl font-semibold text-white'>Peer Comparison</h3>
        </div>
        <div className='grid md:grid-cols-2 gap-6'>
          <div>
            ...
            <div className='text-base font-semibold text-white'>
              {investmentPercentage}% of users in your age/risk group also
              invest in {primaryInvestment || "similar assets"}
            </div>
            <div className='text-white/80 mt-1'>You are one of them! </div>
          </div>
          <div>
            <div className='text-white/80 mb-2 text-lg'>Your Progress</div>
            <div className='text-base font-semibold text-white'>
              You contribute more than {Math.round(100 - percentToGoal * 0.8)}%
              of peers
            </div>
            <div className='text-white/80 mt-1'>
              Your projected payout is in the top{" "}
              {Math.round(100 - percentToGoal + 20)}% for your age group
            </div>
          </div>
        </div>
      </div>
