...
export function AdvancedInsights({
  ...
  const healthStatus = getHealthStatus(
    financialHealth.financial_health_score || 0,
  );
    ...
    <div className='space-y-4 '>
        ...
        <CardContent>
          <div className='space-y-4'>
            ...
            <div className='text-sm text-muted-foreground'>
              Above {financialHealth.peer_percentile?.toFixed(0) || 0}% of your
              peers
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Personalized fund suggestions based on your profile
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className='space-y-3'>
            <div className='text-sm text-muted-foreground'>
              ...
              <span className='font-medium'>
                {fundRecommendations.current_fund || "Unknown"}
              </span>
            </div>
            <div className='text-sm text-muted-foreground'>
              {fundRecommendations.reasoning || "Based on similar users"}
            </div>
            {fundRecommendations.recommendations &&
              fundRecommendations.recommendations.length > 0 && (
                <div>
                  <h4 className='font-semibold mb-2'>Recommended Funds:</h4>
                  <div className='flex flex-wrap gap-2'>
                    {fundRecommendations.recommendations
                      .slice(0, 3)
                      .map((fund: string, index: number) => (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Monte Carlo simulation results ({monteCarlo.simulations || 0}{" "}
            scenarios)
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className='space-y-4'>
            <div className='grid grid-cols-2 md:grid-cols-4 gap-2 text-center'>
              <div>
                ...
                <div className='font-semibold'>
                  ${(monteCarlo.percentiles?.p10 || 0).toLocaleString()}
                </div>
              </div>
              <div>
                <div className='text-xs text-muted-foreground'>
                ...
                <div className='font-semibold'>
                  ${(monteCarlo.percentiles?.p25 || 0).toLocaleString()}
                </div>
              </div>
              <div>
                <div className='text-xs text-muted-foreground'>
                ...
                <div className='font-semibold'>
                  ${(monteCarlo.percentiles?.p50 || 0).toLocaleString()}
                </div>
              </div>
              <div>
                <div className='text-xs text-muted-foreground'>
                ...
                <div className='font-semibold'>
                  ${(monteCarlo.percentiles?.p90 || 0).toLocaleString()}
                </div>
              </div>
            </div>
            <div className='text-center'>
              <div className='text-sm text-muted-foreground'>
                ...
                {((monteCarlo.probability_above_target || 0) * 100).toFixed(1)}%
              </div>
            </div>
          </div>
        </CardContent>
                  ...
                  {peerMatching.peers
                    ...
                    .map((peer: any, index: number) => (
                      <div
                        ...
                        <span>
                          Age {peer.age}, {peer.risk_tolerance} risk
                        </span>
            ...
            {portfolioOptimization.portfolio_metrics && (
              <div className='grid grid-cols-3 gap-2 text-center'>
                <div>
                  ...
                  <div className='font-semibold'>
                    {(
                      portfolioOptimization.portfolio_metrics.expected_return ||
                      0
                    ).toFixed(1)}
                    %
                  </div>
                ...
                <div>
                  ...
                  <div className='font-semibold'>
                    {(
                      portfolioOptimization.portfolio_metrics.volatility || 0
                    ).toFixed(1)}
                    %
                  </div>
                </div>
                <div>
                  ...
                  <div className='font-semibold'>
                    {(
                      portfolioOptimization.portfolio_metrics.sharpe_ratio || 0
                    ).toFixed(2)}
                  </div>
                </div>
              </div>
            )}
            {portfolioOptimization.optimized_allocation &&
              portfolioOptimization.optimized_allocation.length > 0 && (
                <div>
