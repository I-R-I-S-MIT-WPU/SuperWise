...
export function ChatbotPageWithSpeech({ user }: ChatbotPageProps) {
  ...
  const renderFormattedText = (text: string) => {
    ...
    return lines.map((line, lineIdx) => {
      const isHeading = sectionHeaders.some((header) =>
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const userId = (user &&
        (user.User_ID || user.id || user.userId)) as string;
      ...
      const botResponse: ChatMessage = {
        ...
        type: "bot",
        content:
          "I'm sorry, I'm having trouble connecting to the AI service. Please make sure the backend is running.",
        timestamp: new Date(),
      };
                      ...
                      <div
                        ...
                        {message.type === "bot" ? (
                          <div className='space-y-3'>
                            {parseMarkdownTable(message.content).map(
                              (part, idx) => {
                                if (part.type === "table") {
                                  return (
