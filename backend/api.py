...
async def login_user(request: LoginRequest):
    ...
    try:
        ...
        result = await service.login_user(request.username, request.password)
        return result
    except Exception as e:
...
async def forgot_password(request: PasswordResetRequest):
    ...
    try:
        service = SupabaseService()
        result = await service.send_password_reset_email(request.email)
        return result
    except Exception as e:
...
async def reset_password(request: PasswordResetConfirmRequest):
    ...
    try:
        ...
        print(f"Password confirmation error: {e}")
