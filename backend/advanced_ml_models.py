...
class AdvancedMLModels:
    ...
    def load_data(self):
        try:
            ...
            if not response.data:
                print("⚠️  No data found in Supabase database - using empty DataFrame")
            ...
            print(f"⚠️  Error loading data from Supabase: {e}")
            print("⚠️  Continuing with empty DataFrame - authentication still functional")
            self.df = pd.DataFrame()
    ...
    def train_all_models(self):
        # Skip training if no data available (allows API to run without Supabase)
        if self.df.empty:
            print("⚠️  No data available for training - skipping model training")
            print("   Models will be loaded from disk or use fallback predictions")
            return
