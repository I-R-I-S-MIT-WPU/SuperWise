...
class SuperannuationMLPipeline:
    ...
    def load_and_preprocess_data(self):
        ...
        print("Loading data (Supabase with CSV fallback)...")
        ...
        try:
            ...
            # Convert to DataFrame (work on a copy so we don't mutate cached data)
            self.df = df.copy()
            print(
                f"Raw data loaded from {source}: {len(self.df)} users, {len(self.df.columns)} features"
            )
            ...
            print(
                f"Data preprocessed: {len(self.df)} users, {len(self.df.columns)} features"
            )
            return self.df
    ...
    def train_investment_recommendation_model(self):
        ...
        investment_data = self.df[
            investment_features + ["Projected_Pension_Amount"]
        ].dropna()
