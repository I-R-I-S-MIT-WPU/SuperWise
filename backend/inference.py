...
from supabase_config import USER_PROFILES_TABLE, load_user_profiles_dataframe, supabase
...
class SuperannuationInference:
    ...
    def load_models(self):
        ...
        try:
            ...
            self.models["risk_prediction"] = joblib.load(
                f"{self.models_dir}/risk_prediction_model.pkl"
            )
            self.models["investment_recommendation"] = joblib.load(
                f"{self.models_dir}/investment_recommendation_model.pkl"
            )
            self.scaler = joblib.load(f"{self.models_dir}/scaler.pkl")  # For clustering
            self.risk_scaler = joblib.load(
                f"{self.models_dir}/risk_scaler.pkl"
            )  # For risk prediction
            self.investment_scaler = joblib.load(
                f"{self.models_dir}/investment_scaler.pkl"
    ...
    def load_data(self):
        """Load data from Supabase with local CSV fallback"""
        try:
            df, source = load_user_profiles_dataframe()
            if df.empty:
                print("No data found in Supabase or local CSV. Using empty DataFrame.")
                self.df = pd.DataFrame()
                return

            # Work on a copy so we don't mutate cached data
            self.df = df.copy()
            ...
            print(f"Data loaded from {source}: {len(self.df)} users")

        except Exception as e:
            print(f"Error loading data for inference: {e}")
    ...
    def predict_risk_tolerance(self, user_id: str) -> Dict[str, Any]:
        ...
        for feature in risk_features:
            if feature in user:
                ...
                if feature == "Investment_Experience_Level_encoded":
                    ...
                    if (
                        exp_level
                        in self.label_encoders.get(
                            "Investment_Experience_Level", {}
                        ).classes_
                    ):
                        encoded_value = self.label_encoders[
                            "Investment_Experience_Level"
                        ].transform([exp_level])[0]
                    else:
                    ...
                    if debt_level in self.label_encoders.get("Debt_Level", {}).classes_:
                        encoded_value = self.label_encoders["Debt_Level"].transform(
                            [debt_level]
                        )[0]
                    else:
        ...
        risk_probabilities = self.models["risk_prediction"].predict_proba(
            scaled_features
        )[0]
    ...
    def get_user_segment(self, user_id: str) -> Dict[str, Any]:
                ...
                if feature == "Risk_Tolerance":
                    ...
                    if pd.isna(risk_value) or risk_value is None:
                        ...
                        feature_values.append(
                            risk_levels.index(risk_value)
                            if risk_value in risk_levels
                            else 1
                        )
                else:
                    value = user[feature]
        ...
        return {"user_id": user_id, "cluster": int(cluster), "peer_stats": peer_stats}
        ...
        try:
            ...
            if not cluster_users.empty:
                peer_stats = {
                    ...
                    "avg_income": float(cluster_users["Annual_Income"].mean())
                    if "Annual_Income" in cluster_users.columns
                    else 0,
                    "avg_savings": float(cluster_users["Current_Savings"].mean())
                    if "Current_Savings" in cluster_users.columns
                    else 0,
                    "avg_contribution": float(
                        cluster_users["Contribution_Amount"].mean()
                    )
                    if "Contribution_Amount" in cluster_users.columns
                    else 0,
                    "common_investment_types": cluster_users["Investment_Type"]
                    .value_counts()
                    .head(3)
                    .to_dict()
                    if "Investment_Type" in cluster_users.columns
                    else {},
                    "common_fund_names": cluster_users["Fund_Name"]
                    .value_counts()
                    .head(3)
                    .to_dict()
                    if "Fund_Name" in cluster_users.columns
                    else {},
                    "risk_distribution": cluster_users["Risk_Tolerance"]
                    .value_counts()
                    .to_dict()
                    if "Risk_Tolerance" in cluster_users.columns
                    else {},
                    "contribution_percentile": float(
                        (
                            cluster_users["Contribution_Amount"]
                            <= user["Contribution_Amount"]
                        ).mean()
                        * 100
                    )
                    if "Contribution_Amount" in cluster_users.columns
                    and "Contribution_Amount" in user
                    else 50,
                }
            else:
    ...
    def predict_pension_projection(
        self, user_id: str, extra_monthly: float = 0
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for feature in investment_features:
                if feature == "Contribution_Amount":
                    ...
                    if feature == "Risk_Tolerance_encoded":
                        ...
                        if (
                            risk_tolerance
                            in self.label_encoders.get("Risk_Tolerance", {}).classes_
                        ):
                            encoded_value = self.label_encoders[
                                "Risk_Tolerance"
                            ].transform([risk_tolerance])[0]
                        else:
                        ...
                        if (
                            investment_type
                            in self.label_encoders.get("Investment_Type", {}).classes_
                        ):
                            encoded_value = self.label_encoders[
                                "Investment_Type"
                            ].transform([investment_type])[0]
                        else:
            ...
            projected_pension = self.models["investment_recommendation"].predict(
                scaled_features
            )[0]
            ...
            result = {
                ...
                if user.get("Projected_Pension_Amount", 0) is not None
                else 0.0,
                ...
                "improvement": float(
                    projected_pension
                    - (
                        user.get("Projected_Pension_Amount", 0)
                        if user.get("Projected_Pension_Amount", 0) is not None
                        else 0
                    )
                ),
    ...
    def get_summary_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        percent_to_goal = (
            (current_savings / goal_amount * 100) if goal_amount > 0 else 0
        )
        ...
        return {
            ...
            "monthly_income_at_retirement": float(
                user.get("Expected_Annual_Payout", 0) / 12
            )
            if user.get("Expected_Annual_Payout", 0) is not None
            else 0.0,
            "employer_contribution": float(user.get("Employer_Contribution", 0))
            if user.get("Employer_Contribution", 0) is not None
            else 0.0,
            "total_annual_contribution": float(user.get("Total_Annual_Contribution", 0))
            if user.get("Total_Annual_Contribution", 0) is not None
            else 0.0,
